...
//        validateCaptcha(ctx, (String)ctx.getValidatorArg("captcha"), (String)ctx.getValidatorArg("captchaInput"));
        validatePrioridad(ctx, (Integer)beanProps.get("prioridad").getValue());
    }
    ...
    private void validatePrioridad(ValidationContext ctx, int prioridad) {
        if(prioridad <= 0) {
            this.addInvalidMessage(ctx, "prioridad", "Debes establecer una prioridad de envío de mensaje");
        }
    }
